#' ClassifyR: Binary Classification Ensemble with Five Machine Learning Algorithms
#'
#' A comprehensive R package for binary classification tasks using five machine learning
#' algorithms: Support Vector Machines (SVM), Decision Trees, Random Forest, XGBoost,
#' and Neural Networks.
#'
#' The ClassifyR package provides an end-to-end pipeline functionality for:
#' \itemize{
#'   \item Data preprocessing and exploratory data analysis (EDA) for binary classification tasks.
#'   \item Training, tuning, and evaluating various machine learning models (Decision Tree, Random Forest, SVM, XGBoost, Neural Networks).
#'   \item Scratch implementations of foundational algorithms (Neural Networks, Random Forest).
#'   \item Standardized model evaluation metrics, ROC curves, and multi-model comparison visualizations.
#' }
#'
...
#' }
#'
#' @section Algorithms From Scratch:
#' \itemize{
#'   \item \code{trainModel()}: Trains a single-hidden-layer neural network built entirely from scratch with forward and backward propagation.
#'   \item \code{train_rf_scratch()}: Builds an ensemble of decision trees using bootstrap aggregation (bagging) from scratch.
#' }
#'
